function [latent,pcaCoeff,states,state_vectors] = per_animal_PCA(dataObj,sttngs)


    normDat = {};
    normTime = {};
    latent = {};
    pcaCoeff = {};
    states = {};
    state_vectors = {};
    nrns = {};

    for i=1:numel(dataObj)
        %FETCHING DATA
        [normDat{i},normTime{i},states{i},state_vectors{i},nrns{i}] = getData(dataObj,i,sttngs); % fetching and interpolating
        %data_i2 = zscore(data_i, 0, 2);
        
        %CLEANING NEURONS OUT - OPTIONAL
        if sttngs.cleanUpNrns
            if i==4
                disp('stop')
            end
            [normDat{i}] = purge_neurons(normDat{i},nrns{i},sttngs.commonNrns,3);
        end
        
        %NORMALIZATION

        data_i = zscore(normDat{i}, 0, 1); % normalizing the data
        
        %RUNNING THE PCA
        [coeff, score, ~, ~, explained] = pca(data_i', 'NumComponents', 10);
        latent{i} = score';
        pcaCoeff{i} = coeff;
        
        %SMOOTHING THE PC scocres
        st = smoothTrajectory(latent{i},sttngs.kernelSize);
        latent{i} = st(1:3,:);

        %PLOTTING THE RESULTS
        plot_PCA_stats(latent{i},explained,state_vectors{i},states{i},i);
    end


end



function plot_PCA_stats(latent_i,explained,c,states,index)
    figure();
    bar(explained)
    title(['Variance Explained - animal ',num2str(index)])
    


    cmap = jet(max(c));  
    
    
    fig = figure();
    title(['Manifold - animal ',num2str(index)])
    fig.Position = [499   265   733   600];
    

    % Define colormap (adjust if your c values differ)
    

    % Plot initial point
    hold on
    h2 = scatter3(latent_i(1,1), latent_i(2,1), latent_i(3,1), 500, '.', 'CData', [1 0 0]);
    % Set axis limits
    ax = gca;
    axis equal;
    ax = gca;
    

    ax.XLim = [min(latent_i(1,:)), max(latent_i(1,:))];
    ax.YLim = [min(latent_i(2,:)), max(latent_i(2,:))];
    ax.ZLim = [min(latent_i(3,:)), max(latent_i(3,:))];
    % 
    % xl = xlim;
    % yl = ylim;
    % zl = zlim;
    % 
    % plot3([xl(1) xl(2)], [0 0], [0 0], 'k', 'LineWidth',1.5) % X-axis
    % plot3([0 0], [yl(1) yl(2)], [0 0], 'k', 'LineWidth',1.5) % Y-axis
    % plot3([0 0], [0 0], [zl(1) zl(2)], 'k', 'LineWidth',1.5) % Z-axis

    % Animate trajectory
    for i = 2:size(latent_i,2)-1
        % Draw line segment with color from c(i-1)
        plot3(latent_i(1,i-1:i), latent_i(2,i-1:i), latent_i(3,i-1:i), ...
              'Color', cmap(c(i-1),:), 'LineWidth', 2);
 
        % Update moving spot
        h2.XData = latent_i(1,i);
        h2.YData = latent_i(2,i);
        h2.ZData = latent_i(3,i);

    end
    hold off
    
    fig1 = figure();
    fig1.Position = [1251         447         112         413];
    states_vals = 1:max(c);

    imagesc(states_vals');
    colormap(cmap);
    ax1 = gca;
    ax1.YTick = states_vals;
    ax1.YTickLabel= states;


end
